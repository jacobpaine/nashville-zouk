...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const member = await getInstructorBySlug(slug)
  if (!member) return {}
  return {
    title: member.name,
    description: member.bio?.slice(0, 160) ?? `${member.name} — Nashville Zouk community`,
  }
}

export default async function CommunityMemberPage({ params }: Props) {
  const { slug } = await params
  const member = await getInstructorBySlug(slug)
  if (!member) notFound()
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-10">
      <TrackPageView event={{ name: 'community_member_view', data: { slug: member.slug } }} />
      <Link
        href="/community"
